...
public class Main {
	...
	public static void main(String[] args) {
		...
		Machine moore, moore2;
		core.Mealy.Machine mealy;
		...
		for (int i = 0; i < 10000; i++) {
			...
			try {
				moore = new Machine("test", in, out);
				mealy = moore.toMealy();
				moore2 = mealy.toMoore();
				if (!mealy.encode(input).equals(moore.encode(input))) {
					System.out.println("MEALY/MOORE FAILED AT: " + input);
					return;
				}
				if (!moore2.encode(input).equals(moore.encode(input))) {
					System.out.println("MEALY2/MOORE FAILED AT: " + input);
					return;
				}
				if (!mealy.encode(input).equals(moore2.encode(input))) {
